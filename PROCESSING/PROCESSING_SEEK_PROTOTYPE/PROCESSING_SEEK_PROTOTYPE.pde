...
void setup()
{
  ...
  MyButtonController.addButton("Speaker Scale Test")
    .setPosition(300, 15)
    .setSize(100, 20)
    .setId(500)
    ;

  MyButtonController.addToggle("Party Mode")
    .setPosition(420, 15)
    .setSize(100, 20)
    .setValue(false)
...
public class Plant {
  ...
  public void setMaxValue() {
    if (this.plantSensorValue > this.maxValue) {
      this.maxValue = this.plantSensorValue;
    }
  }
...
public void partyMode() {
  if (partyCycle == 5) {
    for (int i = 0; i < partyPlants.length; i++) {
      partyPlants[i] = floor(random(10));
      println("Party plant added: " + partyPlants[i]);
    }
    partyCycle--;
  }
  if (partyCycle < 5) {
    if (millis() > cycleTime + cycleInterval) {
      for (int i = 0; i < partyPlants.length; i++) {
        sendOSCMessage(plants[partyPlants[i]].plantName, floor(random(15)));
      }
      println("Current party cycle: " + partyCycle);
      partyCycle--;
