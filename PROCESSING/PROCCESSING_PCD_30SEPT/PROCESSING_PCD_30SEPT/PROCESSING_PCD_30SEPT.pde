...
void setup()
{
  ...
  MyButtonController.addButton("Calibrate")
    .setPosition(820, 15)
    .setSize(100, 20)
    .setId(600)
    ;
...
void plantBuilder() {
  ...
  if (!debugMode) {
    ...
    for (int i = 0; i < plants.length; i++) {
      ...
      plants[i].buildSlider();
      plants[i].buildText();
      plants[i].plantSensorValue = 10000;
      println("Build plant object: " + plants[i].plantName + " with ID: " + plants[i].id);
      delay(50);
    }
  }
...
public class Plant {
  ...
  public int[] pastSensorValues = new int [sensorCalibrationAmount];
...
public void generateDebugSensorValue() {
  if (millis() % 2 == 0) {
    for (int i = 0; i < plants.length; i++) {
      plants[i].plantSensorValue = plants[i].plantSensorValue + floor(random(-5, 5));
      if (plants[i].plantSensorValue <= 0) {
        plants[i].plantSensorValue = 0;
      }
    }
  }
}
